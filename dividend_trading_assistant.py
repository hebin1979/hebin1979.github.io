...
SNAPSHOT = {
    "as_of": "2026-08-24",
    "pe": 14.05,            # 沪深300 PE(TTM) 2026-08-24
    "bond10y": 1.685,       # 10Y 国债收益率 (%) 中债 2026-08-21/24
    "div_yield": 4.52,      # 563020 股息率 (%)（中证红利低波动930914指数口径）
    "etf_price": 1.179,     # 563020 现价(不复权, gtimg 2026-08-24)
    "etf_52w_high": 1.283,  # 52周高 (2025-11-12, 不复权)
    "etf_52w_low": 1.055,   # 52周低 (2026-06-29, 不复权)
    "etf_ma": 1.1842,       # 中期均线 MA200 日线(不复权)
    "etf_rsi": 60.1,        # RSI(14) 日线(不复权)
}
...
# ----------------------------------------------------------------------------
SHARED_MACRO = {
    "pe": 14.05,            # 沪深300 PE(TTM) —— 共享宏观估值背景(2026-08-24)
    "bond10y": 1.685,       # 10Y 国债收益率(%) —— 共享宏观利率背景(中债 2026-08-21/24)
}
FUNDS = [
    {
        "code": "563020",
    ...
    },
    {
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.331,    # 159545 现价(不复权, gtimg 2026-08-24)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权)
            "etf_ma": 1.4124,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 57.41,      # RSI(14) 日线(不复权)
        }),
    },
]

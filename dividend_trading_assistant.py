...
SNAPSHOT = {
    "as_of": "2026-08-31",
    "pe": 13.65,            # 沪深300 PE(TTM) 2026-08-31
    "bond10y": 1.84,        # 10Y 国债收益率 (%) 中债 2026-08-29
    "div_yield": 4.52,      # 563020 股息率 (%)（中证红利低波动930914指数口径）
    "etf_price": 1.187,     # 563020 现价(不复权, gtimg 2026-08-31)
    "etf_52w_high": 1.283,  # 52周高 (2025-11-12, 不复权)
    "etf_52w_low": 1.055,   # 52周低 (2026-06-29, 不复权)
    "etf_ma": 1.1828,       # 中期均线 MA200 日线(不复权)
    "etf_rsi": 61.72,       # RSI(14) 日线(不复权)
}
...
# ----------------------------------------------------------------------------
SHARED_MACRO = {
    "pe": 13.65,            # 沪深300 PE(TTM) —— 共享宏观估值背景(2026-08-31)
    "bond10y": 1.84,        # 10Y 国债收益率(%) —— 共享宏观利率背景(中债 2026-08-29)
}
FUNDS = [
    {
        "code": "563020",
    ...
    },
    {
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.318,    # 159545 现价(不复权, gtimg 2026-08-31)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权)
            "etf_ma": 1.4086,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 51.37,      # RSI(14) 日线(不复权)
        }),
    },
]

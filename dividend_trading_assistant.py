...
SNAPSHOT = {
    "as_of": "2026-09-14",
    "pe": 13.50,            # 沪深300 PE(TTM) 2026-09-14
    "bond10y": 1.69,        # 10Y 国债收益率 (%) 中债曲线 2026-09-11
    "div_yield": 4.52,      # 563020 股息率 (%)（中证红利低波动930914指数口径）
    "etf_price": 1.195,     # 563020 现价(不复权, gtimg 2026-09-07)
    "etf_52w_high": 1.283,  # 52周高 (不复权)
    "etf_52w_low": 1.055,   # 52周低 (不复权)
    "etf_ma": 1.1812,       # 中期均线 MA200 日线(不复权)
    "etf_rsi": 63.2,        # RSI(14) 日线(不复权)
}
...
# ----------------------------------------------------------------------------
SHARED_MACRO = {
    "pe": 13.50,            # 沪深300 PE(TTM) —— 共享宏观估值背景(2026-09-14)
    "bond10y": 1.69,        # 10Y 国债收益率(%) —— 共享宏观利率背景(中债曲线 2026-09-11)
}
FUNDS = [
    {
        "code": "563020",
        ...
        "snapshot": dict(SHARED_MACRO, **{
            "div_yield": 4.52,     # 563020 股息率(%)（中证红利低波动930914指数口径）
            "etf_price": 1.186,    # 563020 现价(不复权, gtimg 2026-09-14)
            "etf_52w_high": 1.283, # 52周高 (不复权)
            "etf_52w_low": 1.055,  # 52周低 (不复权)
            "etf_ma": 1.1795,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 52.9,       # RSI(14) 日线(不复权)
        }),
    },
    {
        "code": "159545",
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.317,    # 159545 现价(不复权, gtimg 2026-09-14)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权)
            "etf_ma": 1.3985,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 51.4,       # RSI(14) 日线(不复权)
        }),
    },
]

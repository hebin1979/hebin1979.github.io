...
FUNDS = [
    {
        ...
        "snapshot": dict(SHARED_MACRO, **{
            "div_yield": 4.52,     # 563020 股息率(%)（中证红利低波动930914指数口径）
            "etf_price": 1.155,    # 563020 现价(不复权, 东财 2026-07-24)
            "etf_52w_high": 1.283, # 52周高 (2025-11-12, 不复权)
            "etf_52w_low": 1.055,  # 52周低 (2026-06-29, 不复权)
            "etf_ma": 1.1883,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 57.83,      # RSI(14) 日线(不复权)
        }),
    },
    {
        "code": "159545",
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.325,    # 159545 现价(不复权, 东财 2026-07-24)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权；现价贴近1年低位)
            "etf_ma": 1.4243,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 60.21,      # RSI(14) 日线(不复权)
        }),
    },
]

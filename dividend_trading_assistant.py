...
SNAPSHOT = {
    "as_of": "2026-09-07",
    "pe": 13.65,            # 沪深300 PE(TTM) 2026-09-07
    "bond10y": 1.68,        # 10Y 国债收益率 (%) 中债曲线 2026-09-04
...
# ----------------------------------------------------------------------------
SHARED_MACRO = {
    "pe": 13.65,            # 沪深300 PE(TTM) —— 共享宏观估值背景(2026-09-07)
    "bond10y": 1.68,        # 10Y 国债收益率(%) —— 共享宏观利率背景(中债曲线 2026-09-04)
}
FUNDS = [
    {
        "code": "563020",
    ...
    },
    {
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.315,    # 159545 现价(不复权, gtimg 2026-09-07)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权)
            "etf_ma": 1.4034,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 50.0,       # RSI(14) 日线(不复权)
        }),
    },
]

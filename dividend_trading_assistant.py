...
SNAPSHOT = {
    "as_of": "2026-08-20",
    "pe": 14.20,            # 沪深300 PE(TTM) 蛋卷 08-19
    "bond10y": 1.69,        # 10Y 国债收益率 (%) 沿用 08-18
    "div_yield": 4.52,      # 563020 股息率 (%)（中证红利低波动930914指数口径）
    "etf_price": 1.174,     # 563020 现价(不复权, gtimg 2026-08-20)
    "etf_52w_high": 1.283,  # 52周高 (2025-11-12, 不复权)
    "etf_52w_low": 1.055,   # 52周低 (2026-06-29, 不复权)
    "etf_ma": 1.1847,       # 中期均线 MA200 日线(不复权)
    "etf_rsi": 59.7,        # RSI(14) 日线(不复权)
}
...
# ----------------------------------------------------------------------------
SHARED_MACRO = {
    "pe": 14.20,            # 沪深300 PE(TTM) —— 共享宏观估值背景(蛋卷 2026-08-19)
...
FUNDS = [
    ...
    },
    {
        ...
        "snapshot": dict(SHARED_MACRO, **{
            ...
            "etf_price": 1.327,    # 159545 现价(不复权, gtimg 2026-08-20)
            "etf_52w_high": 1.541, # 52周高 (不复权)
            "etf_52w_low": 1.193,  # 52周低 (不复权)
            "etf_ma": 1.4137,      # 中期均线 MA200 日线(不复权)
            "etf_rsi": 57.24,      # RSI(14) 日线(不复权)
        }),
    },
]

...
SNAPSHOT = {
    ...
    "gold_price": 8.482,            # 518850 现价 (元)
    "gold_52w_high": 12.087,
    "gold_52w_low": 7.310,
    "gold_ma": 9.5533,              # 长期均线(约 200 日/月线收盘均值)
    "gold_rsi": 43.0,
...
def analyze(d):
    ...
    # —— 信号由「价位区间」驱动（与红利/纳指一致）：方向看价格分位，评分/环境分仅作力度注脚 ——
    ...
    if zone_name == "深度价值区":
        advice = (f"黄金处于「深度价值区」(52周分位 {gold_pct:.0f}%，贴年内低位)，安全边际最高。"
                  f"建议重仓累积、越跌越买；实际利率回落或美元走弱可加仓。")
    elif zone_name == "价值区":
        advice = (f"黄金处于「价值区」(52周分位 {gold_pct:.0f}%)，偏低、有吸引力。"
                  f"建议逢低建仓、分批买入，不一次性满仓；环境分 {env:.0f} 作力度参考。")
    elif zone_name == "合理区":
        advice = (f"黄金处于「合理区」(52周分位 {gold_pct:.0f}%)，估值合理、历史中枢。"
                  f"持有现有仓位、定投保值为主，不追高；环境分 {env:.0f}。")
    elif zone_name == "偏高区":
        advice = (f"黄金处于「偏高区」(52周分位 {gold_pct:.0f}%)，接近年内高位。"
                  f"持有但分批止盈，逼近前高({d['gold_52w_high']:.3f})分批减仓，不宜追高；环境分 {env:.0f}。")
    else:  # 高估区
        advice = (f"黄金处于「高估区」(52周分位 {gold_pct:.0f}%)，逼近/刷新52周高位、性价比下降。"
                  f"建议分批减仓、获利了结，仅留小额底仓对冲；不追高，等待回撤至价值区再分批回补。"
...
def reconcile_text(d, ctx, zone_name, gold_pct):
    ...
    buy_zone = gold_pct < 40
    pos = ("处于年内偏低位置、安全边际高" if gold_pct < 40 else
    ...
    if ind["season"] > 70: bull.append(f"处于旺季(季节评分 {ind['season']:.0f})实物需求支撑")
    if buy_zone:
...
<div class="section"><h2>② 价位区间参考（仅标示价格相对 52 周区间的位置，安全边际参考）</h2>
  ...
  黄金长期保值属性强，但高估区应"分批减仓、获利了结、仅留小额底仓对冲"，不一次清仓；回撤至价值区再分批回补。建议分批操作。</p>
</div>

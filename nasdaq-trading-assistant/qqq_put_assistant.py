...
评分机制（两层）：价格(估值分位)锚定买卖方向占 60%，其余 6 指标作为环境分调制力度占 40%；
  信号由「价位区间」(深度价值/价值/合理/偏高/高估) 直接驱动方向，综合评分/环境分仅作力度注脚——
  故价位区间与信号方向必然一致（便宜→偏多、贵→偏谨慎），不再用评分阈值单独判买卖。
信号（按价位区间，单调梯度，与红利/黄金引擎一致）：
  深度价值区  强烈买入 · 重仓累积
  价值区      逢低建仓 · 分批买入
  合理区      持有 · 定投收息
  偏高区      持有 · 分批止盈
...
SNAPSHOT = {
    ...
    "etf_price": 1.904, "etf_52w_high": 2.168, "etf_52w_low": 1.515,
    "etf_ma": 1.7945, "etf_rsi": 35.8,
...
def analyze(d):
    ...
    # —— 信号由「价位区间」驱动（与红利/黄金一致）：方向看价格分位，评分/环境分仅作力度注脚 ——
    zone_name, zone_color, zone_desc = current_zone(d)
    ZONE_SIG = {
        "深度价值区": ("强烈买入 · 重仓累积", "#16a34a"),
        "价值区":     ("逢低建仓 · 分批买入", "#65a30d"),
        "合理区":     ("持有 · 定投收息",     "#d97706"),
        "偏高区":     ("持有 · 分批止盈",     "#ea580c"),
        "高估区":     ("建议减仓 · 获利了结", "#dc2626"),
    }
    signal, color = ZONE_SIG[zone_name]
    if zone_name == "深度价值区":
        advice = (f"纳指处于「深度价值区」(52周分位 {etf_pct:.0f}%，贴年内低位)，安全边际最高。"
                  f"建议重仓累积、越跌越买；急跌至强支撑(长期均线 {d['etf_ma']:.3f})可加仓。"
                  f"环境分 {env:.0f}、综合评分 {composite:.0f} 作力度参考。")
    elif zone_name == "价值区":
        advice = (f"纳指处于「价值区」(52周分位 {etf_pct:.0f}%)，偏低、有吸引力。"
                  f"建议逢低建仓、分批买入，不一次性满仓；环境分 {env:.0f} 作力度参考。")
    elif zone_name == "合理区":
        advice = (f"纳指处于「合理区」(52周分位 {etf_pct:.0f}%)，估值合理、历史中枢。"
                  f"持有现有仓位、定投收息为主，不追高；环境分 {env:.0f}。")
    elif zone_name == "偏高区":
        advice = (f"纳指处于「偏高区」(52周分位 {etf_pct:.0f}%)，接近年内高位。"
                  f"持有但分批止盈，逼近前高({d['etf_52w_high']:.3f})分批减仓，不宜追高；环境分 {env:.0f}。")
    else:  # 高估区
        advice = (f"纳指处于「高估区」(52周分位 {etf_pct:.0f}%)，逼近/刷新52周高位、性价比下降。"
                  f"建议分批减仓、获利了结，仅留小额底仓；不追高，等待回撤至合理区再分批回补。"
                  f"环境分 {env:.0f}、综合评分 {composite:.0f} 仅作力度参考。")
...
def reconcile_text(d, ctx, zone_name, etf_pct):
    """两层框架的桥梁：信号由价位区间驱动，综合评分/环境分只调制力度（不反转方向）。
    便宜→偏多、贵→偏谨慎；环境好则力度更足，环境差则减小。"""
    ...
    if ctx["vix_pct"] < 20: bear.append(f"VIX 处低位({ctx['vix_pct']:.0f}%分位)，追高性价比低")
    bull = []
    if ctx["trend_ratio"] > 1.05: bull.append(f"趋势仍站上长期均线({ctx['trend_ratio']:.2f}×) 顺势")
    ...
    if buy_zone:
        ...
        return (f"当前 159696 现价 {d['etf_price']:.3f}，价位「{zone_name}」{pos}（52 周区间分位 {etf_pct:.0f}%）。"
                f"价格处低位→方向偏多；信号「{sig}」由价位区间驱动。环境分 {env:.0f}、综合评分 {comp:.0f} 仅调制力度："
                f"虽便宜可逢低布局，但{tail}。")
    tail = ("；".join(bull) + "，但价格已偏高/高估、安全边际有限，故以减仓/止盈为主、不追高") if bull else "，且价格偏高/高估、安全边际有限，故以减仓/止盈为主、不追高"
    return (f"当前 159696 现价 {d['etf_price']:.3f}，价位「{zone_name}」{pos}（52 周区间分位 {etf_pct:.0f}%）。"
            f"价格偏高→方向偏谨慎；信号「{sig}」由价位区间驱动。环境分 {env:.0f}、综合评分 {comp:.0f} 仅调制力度：{tail}。")
...
<div class="section"><h2>② 价位区间参考（仅标示价格相对 52 周区间的位置，安全边际参考）</h2>
  ...
  纳指长期向上，但高估区应"分批减仓、获利了结、仅留小额底仓"，不一次清仓；回撤至合理区再分批回补。建议分批操作。</p>
</div>
